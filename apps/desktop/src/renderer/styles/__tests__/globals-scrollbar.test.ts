...
describe('global scrollbar styles', () => {
  it('keeps native scrollbars hidden until a scroll container is active', () => {
    expect(globalsCss).toContain('--vividrop-scrollbar-thumb: transparent');
    expect(globalsCss).toContain('--vividrop-scrollbar-thumb-active');
    expect(globalsCss).toContain('scrollbar-color: var(--vividrop-scrollbar-thumb) transparent');
    expect(globalsCss).toContain('.vividrop-scrollbar-active');
    expect(globalsCss).toContain('::-webkit-scrollbar-thumb');
    expect(globalsCss).toContain('background-color: var(--vividrop-scrollbar-thumb)');
    expect(globalsCss).toContain('background-color: var(--vividrop-scrollbar-thumb-active)');
  });
});
...
describe('global text selection styles', () => {
  it('disables incidental UI text selection while keeping copyable content selectable', () => {
    ...
    expect(globalsCss).toContain('.vividrop-selectable-text');
