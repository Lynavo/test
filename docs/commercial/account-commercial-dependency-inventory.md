...
| Support/update backend  | Desktop support/update network paths were removed; diagnostics now export a local archive only. The old unmounted support panel, reset-state IPC, and reset support copy were removed. | Removed from OSS | Keep official remote diagnostic submission, desktop updater clients, and destructive support reset entrypoints out of OSS. |
...
| Local diagnostics export           | Mobile diagnostics now routes through `shareDiagnosticsArchive`, which asks the native engine for a local archive and opens the system share sheet.                   | Keep baseline         | Keep local export/share only; do not add official remote diagnostic submission APIs.                                                     |
...
| Build profile drift     | `AGENTS.md`, `docs/release/build-profile-flow.md`, and `docs/release/release-playbook.md` must stay aligned that OSS release profiles stay build-only.                                                                                                                                     | Harden         | Keep current docs aligned with OSS build-only release profiles.       |

## Recommended Follow-up Order

1. Keep release documentation aligned on the OSS boundary: current
   `review` / `prod` release profiles are build verification selectors only,
   and official signing, upload, update hosting and beta-tag automation stay
...
3. Use the captured OSS beta smoke evidence in
   ...
   and removed remote diagnostics/update/reset-state runtime entrypoints. Complete
