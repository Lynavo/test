...
| Service endpoints          | `packages/contracts/src/service-endpoints.ts:1` exports web, support API and support email scalars only. Broad `LYNAVO_API_BASE_URL` / aggregate endpoint exports were removed from contracts; release/dev profiles now emit `LYNAVO_SUPPORT_API_BASE_URL`. | Keep support/update baseline | Keep official account, entitlement, auth and tunnel API constants out of public contracts.                                          |
...
1. Decide whether community OSS keeps one-click diagnostic upload to Lynavo
   support APIs. If not, convert mobile and desktop diagnostics to local
   export/share/email only.
2. Add positive support IPC/preload tests for diagnostics export/upload and
   update checks, complementing the existing negative commercial bridge tests.
